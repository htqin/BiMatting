...
def matting_loss(pred_fgr, pred_pha, true_fgr, true_pha, lvl=5):
    """
    Args:
    ...
    loss['pha_laplacian'] = laplacian_loss(pred_pha.flatten(0, 1), true_pha.flatten(0, 1), lvl)
...
def laplacian_pyramid(img, kernel, max_levels):
    current = img
    #print('current size:', current.shape)
    pyramid = []
...
def save_model_output(module, input, output):
...
def distill_loss(layers, teacher_layers):
    loss_distill = 0
    for ((name1, layer1), (name2, layer2)) in zip(layers, teacher_layers):
        # print(name1, name2, len(layer1.output), len(layer2.output))
        for i in range(len(layer1.output)):
            # print(name1, name2, i, layer1.output[i].shape, layer2.output[i].shape)
            loss_distill += mse(layer1.output[i], layer2.output[i])
        layer1.output = []
        layer2.output = []
    return loss_distill
