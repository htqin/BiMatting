...
class BiDeepGuidedFilterRefiner(nn.Module):
    def __init__(self, hid_channels=16):
        ...
        self.conv = nn.Sequential(
            LearnableBias(4 * 2 + hid_channels),
            BinaryActivation(),
            BiConv2d(4 * 2 + hid_channels, hid_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(hid_channels),
            LearnableBias(hid_channels),
            nn.PReLU(hid_channels),
            LearnableBias(hid_channels),
            BinaryActivation(),
            BiConv2d(hid_channels, hid_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(hid_channels),
            LearnableBias(hid_channels),
            nn.PReLU(hid_channels),
