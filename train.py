...
from train_loss import matting_loss, segmentation_loss, distill_loss, save_model_output
...
class Trainer:
    ...
    def parse_args(self):
        ...
        # Model
        parser.add_argument('--model-variant', type=str, required=True, choices=['mobilenetv3', 'resnet50', 'bimobilenet'])
        # Matting dataset
        parser.add_argument('--dataset', type=str, required=True, choices=['videomatte', 'imagematte'])
        parser.add_argument('--prefix', type=str, default='/cluster/work/cvl/haoqin/datasets/matting-data')
        # Learning rate
        ...
        parser.add_argument('--distributed-port', type=str, default='12388')
        # Debugging
    ...
    def init_datasets(self):
        ...
        for dataset in DATA_PATHS.keys():
            for dir in DATA_PATHS[dataset].keys():
                DATA_PATHS[dataset][dir] = DATA_PATHS[dataset][dir].replace('/cluster/work/cvl/haoqin/datasets/matting-data', self.args.prefix)
        ...
        if self.args.dataset == 'videomatte':
            ...
            self.dataset_valid = VideoMatteDataset(
                videomatte_dir=DATA_PATHS['videomatte_SD']['valid'],
    ...
    def init_model(self):
        ...
        if self.args.distill:
            self.teacher_model = MattingNetwork(self.args.model_variant).to(self.rank)
            self.teacher_model = nn.SyncBatchNorm.convert_sync_batchnorm(self.teacher_model)
            self.teacher_model_ddp = DDP(self.teacher_model, device_ids=[self.rank], broadcast_buffers=False, find_unused_parameters=True)
            self.layers = [(name, module) for name, module in self.model_ddp.named_modules() if str(type(module))[8:-2].split('.')[-1] in ['HardBinaryConv', 'Conv2d']]
            self.teacher_layers = [(name, module) for name, module in self.teacher_model_ddp.named_modules() if str(type(module))[8:-2].split('.')[-1] in ['HardBinaryConv', 'Conv2d']]
            # self.layers, self.teacher_layers = zip(*[((name1, module1), (name2, module2)) for (name1, module1), (name2, module2) in zip(self.layers, self.teacher_layers) if type(module1) != type(module2)])
            for name, module in self.layers:
                module.register_forward_hook(save_model_output)
                module.output = []
            for name, module in self.teacher_layers:
                module.register_forward_hook(save_model_output)
                module.output = []
    ...
    def train(self):
        for epoch in range(self.args.epoch_start, self.args.epoch_end):
            ...
            for true_fgr, true_pha, true_bgr in tqdm(self.dataloader_lr_train, disable=self.args.disable_progress_bar or self.rank != 0, dynamic_ncols=True):
                # Low resolution pass
    ...
    def train_mat(self, true_fgr, true_pha, true_bgr, downsample_ratio, tag):
        ...
        with autocast(enabled=not self.args.disable_mixed_precision):
            pred_fgr, pred_pha, pred_fgr_s, pred_pha_s = self.model_ddp(true_src, downsample_ratio=downsample_ratio)[:4]
            true_fgr_s = F.interpolate(true_fgr.flatten(0, 1), size=pred_fgr_s.shape[-2:], mode='bilinear', align_corners=False).unflatten(0,(true_fgr.shape[0], true_fgr.shape[1]))
            true_pha_s = F.interpolate(true_pha.flatten(0, 1), size=pred_pha_s.shape[-2:], mode='bilinear', align_corners=False).unflatten(0,(true_pha.shape[0], true_pha.shape[1]))
            #print('normal:', pred_fgr.shape, pred_pha.shape, true_fgr.shape, true_pha.shape)
            #print('small:', pred_fgr_s.shape, pred_pha_s.shape, true_fgr_s.shape, true_pha_s.shape)
            loss = matting_loss(pred_fgr, pred_pha, true_fgr, true_pha) 

            loss['total'] += matting_loss(pred_fgr_s, pred_pha_s, true_fgr_s, true_pha_s, 1)['total'] * 0.25

            if self.args.distill:
                self.teacher_model(true_src, downsample_ratio=downsample_ratio)[:2]
                loss['distill'] = distill_loss(self.layers, self.teacher_layers)
                loss['total'] += loss['distill']
        ...
        if self.rank == 0 and self.step % (self.args.log_train_loss_interval * 20) == 0:
    ...
    def train_seg(self, true_img, true_seg, log_label):
        ...
        with autocast(enabled=not self.args.disable_mixed_precision):
            ...
            if self.args.distill:
                self.teacher_model(true_img, segmentation_pass=True)[0]
                loss += distill_loss(self.layers, self.teacher_layers)
    ...
    def random_crop(self, *imgs):
        h, w = imgs[0].shape[-2:]
        w = random.choice(range(w // 2, w)) // 16 * 16
        h = random.choice(range(h // 2, h)) // 16 * 16
        results = []
        for img in imgs:
            B, T = img.shape[:2]
            img = img.flatten(0, 1)
