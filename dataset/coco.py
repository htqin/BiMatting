...
class CocoPanopticTrainAugmentation:
    ...
    def __call__(self, img, seg):
        ...
        img = F.affine(img, *params)
        seg = F.affine(seg, *params)
        
        # Resize
        params = transforms.RandomResizedCrop.get_params(img, scale=(0.5, 1), ratio=(0.7, 1.3))
        img = F.resized_crop(img, *params, self.size, interpolation=PIL.Image.BILINEAR)
        seg = F.resized_crop(seg, *params, self.size, interpolation=PIL.Image.NEAREST)
